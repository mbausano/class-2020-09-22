...
-What do we want to know: THe change in enrollment bewteen classes over a three week period
- We have the data from the weeks ago and from 4 days ago
-We want to compare the two data sets and calculate a rate of change

After we have a model, Temperance suggests we should:
- Consider how this drop might relate to other drops in the history of this time period
- Determine whether what we have found is actually of statistical significance
- Think if this is just a consequence of the add/drop period

```{r}
joint_environment <-
  inner_join(x=sep_old, y=sep_new, by= c("id", "department", "title", "name")) %>%
  mutate(enrollmentchange= enrollment.y-enrollment.x) 
    arrange(joint_environment, by= enrollmentchange)
```
